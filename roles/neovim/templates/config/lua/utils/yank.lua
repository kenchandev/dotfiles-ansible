...
M.copy_path = function(path, label)
  vim.fn.setreg('+', path)
  vim.notify('Yanked ' .. label .. ' path: ' .. path, vim.log.levels.INFO)
end

M.copy_visual_with_path = function(path, label)
  local bounds = M.get_visual_bounds()
  ...
  M.simulate_highlight()

  M.exit_visual_mode()

  vim.notify('Yanked ' .. label .. ' with lines ' .. line_range, vim.log.levels.INFO)
end
